...
最近，我们一直在探索一些提议，将这一规则的一部分 —— [包括契约](https://www.youtube.com/watch?v=Hjz1eBx91g8) —— 带到 C++ 中。也有将其中的一些特性也带到 C# 中的[具体提议](https://github.com/dotnet/roslyn/issues/119)。我们正在积极地对一个[将一些 non-null 检查带到 C# 的提议](https://github.com/dotnet/roslyn/issues/5032)进行迭代。我不得不承认，我希望所有的这些提议都是最好的，然而没有什么能够跟整个栈都用同样的错误规则实现那样刀枪不入。而且得记住，整个隔离和并发模型对于大规模的丢弃是必不可少的。
