...
一开始，我们没有支持跨越了异步边界的异常。然而最终，我们扩展了声明 throws 的能力以及可选的类型化异常的子句，使它们跨越异步过程边界。这给异步的 actor 编程模型带来了一个丰富的类型化的编程模型并感觉就像一个自然的扩展。这从 CLU 的继任者 [Argus](https://en.wikipedia.org/wiki/Argus_(programming_language)) 那里借鉴了不少。

我们的诊断基础结构美化了这点，给开发人员在他们的堆栈视图中全面查看跨过程的因果关系的调试体验。不是仅仅是高并发系统中的栈仙人掌，而且经常模糊在跨越过程消息传递的边界。可以这样调试系统可以节省大量的时间。
