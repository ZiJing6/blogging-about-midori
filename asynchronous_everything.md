...
我们改进的执行模型的另一个绝大的优点是栈又回来了！是的，你实际上得到了等待多层次深度的异步活动的栈跟踪，必须要额外的开销。许多像 .NET 的系统不得不用它们的方式从一大堆散乱的类栈的对象中抽出一片片收集拼凑出一个栈帧。我们遇到了跨进程的挑战，但在一个单一的进程中，所有的活动都有正常的栈跟踪和处于良好状态的变量。
