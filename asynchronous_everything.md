...
我们创建的模型是一个异步活动跑在[链接栈（linked stack）](https://gcc.gnu.org/wiki/SplitStacks)上的模型。这些链接开始只有 128 字节那么小，并且只会根据需要增长。多次试验之后，我们形成了个链接每次增长一倍大小的模型。即是一开始链接是 128b，然后 256b，一直到 8k 的分块。实现这个需要很深入的编译器支持。真的需要这样做将它做好。编译器知道提升链接检查，特别是提升到循环之外，并在它能预测栈帧的大小的时候探测更大的总量（导致内联）。用链接的一个常见问题是最后你会频繁重新链接，特别是在循环里进行函数调用的边界，然而上述的绝大部分优化避免了这种情况发生。而且即使这发生了，我们的链接代码是手写的汇编 —— 如果我没记错的话，它用三个指令去连接 —— 而且我们保持一个热连接段的后备以供重用。  【这段翻译得超烂，我也不是很懂。】
