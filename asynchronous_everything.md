...
流的实现处理了大量和通用的细节，确保流是尽可能高效的。一个关键的技术是流控，从 TCP 那边借鉴过来的。一个流的生产者和消费者完全在抽象的覆盖下合作，以确保管道不会变得非常不平衡。这很像 TCP 流控的工作方式，维护一个叫做窗口（window）的东西，并根据情况打开或关闭它。总的来说，这非常有效。例如，我们的实时多媒体栈有两个异步的管道，一个处理音频，另一个处理视频，并将它们合并在一起，以实现音频和视频的同步。通常，内置的流控机制就能让他们不掉帧。
