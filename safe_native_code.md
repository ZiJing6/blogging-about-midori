...
一个 top-level 的决定是：是要_保守_，还是要_精确_？一个保守的收集齐更容易切入现有的系统，但它会在某些方面造成麻烦。它经常需要扫描更多的堆来完成同样的工作。而且它会错误地保持对象存活。我们觉得这两点对于系统编程环境是都不能接受的。这是一个简单快速的决定：我们追求精确。
...
这引出了_协作(cooperative)_ 对比 _抢占(preemptive)_的主题，以及 GC 安全点的概念。