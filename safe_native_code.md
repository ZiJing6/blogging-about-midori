...
泛型实例化知识一些花哨的带一些替代的代码的复制和粘贴。很显然，与开发人员实际编写的代码相比，这意味着编译器要处理的代码激增。[我之前已经写过很多关于泛型的性能挑战](http://joeduffyblog.com/2011/10/23/on-generics-and-some-of-the-associated-overheads/)。一个重要的问题是传递闭包问题。.NET 种看起来简单直接的 List&lt;T> 实际上在它的传递闭包种创建了 28 种类型！还没说每个类型种的所有方法。泛型是代码大小快速激增的原因。
