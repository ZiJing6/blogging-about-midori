...
震惊吧，这一切都奏效了！我之前提到过的那些案例 —— 图像解码器、多媒体栈、浏览器等等 —— 除了在构建为使用许多并行进程中之外，现在可以使用安全的进程内并行了。更有趣的是，我们的一个产品负载 —— 处理 Bing.com 的语音识别流量 —— 实际上显著降低了延迟并因此提高了吞吐量。事实上，Cartana（小娜）的基于 DNN 的语音识别算法，大大提高了准确性，在用这种整体并行模型之前，可能从来没有达到过它的延迟目标。
